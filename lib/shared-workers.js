...
function addTestWorker(testWorker) {
	testWorker.onRegisterSharedWorker(async ({channelId, workerId, options}) => {
		const sharedWorker = await register(workerId, options);
		sharedWorker.on('message', async value => {
			// TODO: Allow messages to be broadcast to all test workers, or sent to a
			// specific worker.

			testWorker.passSharedWorkerMessage({
				channelId,
				value: v8.serialize(value).toString('base64')
			});
		});

		testWorker.onPassSharedWorkerMessage(({channelId: dest, value}) => {
			if (channelId === dest) {
				sharedWorker.postMessage(value);
			}
		});
