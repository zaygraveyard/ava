...
module.exports = (file, options, execArgv = process.execArgv) => {
		...
		subprocess.on('message', message => {
			...
			switch (message.ava.type) {
				...
				case 'register-shared-worker':
					emitter.emit('registerSharedWorker', message.ava);
					break;
				case 'pass-shared-worker-message':
					emitter.emit('passSharedWorkerMessage', message.ava);
					break;
				case 'ping':
					send({type: 'pong'});
					break;
	...
	return {
		...
		onRegisterSharedWorker(listener) {
			return emitter.on('registerSharedWorker', listener);
		},

		onPassSharedWorkerMessage(listener) {
			return emitter.on('passSharedWorkerMessage', ({value, ...message}) => {
				listener({
					...message,
					value: v8.deserialize(Buffer.from(value, 'base64'))
				});
			});
		},
		...
		passSharedWorkerMessage(message) {
			send({...message, type: 'pass-shared-worker-message'});
		},
