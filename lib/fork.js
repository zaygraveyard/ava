...
const serialization = require('./data-serialization');
...
class SharedWorkerChannel extends Emittery {
	constructor({channelId, filename, initialData}, send) {
		super();

		this.id = channelId;
		this.filename = filename;
		this.initialData = serialization.deserializeData(initialData);
		this.send = send;
	}
	...
	receiveMessage({messageId, replyTo, data}) {
		this.emit('message', {
			...
			messageId,
			replyTo,
			data: serialization.deserializeData(data)
		});
	}
	...
	postMessage({messageId, replyTo, data}) {
		this.send({
			...
			messageId,
			replyTo,
			data: serialization.serializeData(data)
		});
	}
}
