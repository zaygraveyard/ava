...
ipc.options.then(async options => {
	...
	exports.accessRunner = () => {
		accessedRunner = true;
		return runner;
	};

	exports.registerSharedWorker = (filename, initialOptions, teardown) => {
		const state = ipc.registerSharedWorker(filename, initialOptions);
		runner.waitForReady.push(state.ready);
		sharedWorkerTeardowns.push(teardown);
		return state;
	};
