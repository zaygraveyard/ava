...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.createWorkerChannel = options.createWorkerChannel;
		...
		const meta = Object.freeze({
			...
			register: async plugin => this.registerPlugin(plugin)
		});
	...
	async registerPlugin(plugin) {
		// TODO: Check type
		// TODO: Use existing registration based on ID
		// TODO: Validate plugin object.
		const channel = this.createWorkerChannel(plugin);

		let dispose;
		try {
			channel.ref();
			return await new Promise(resolve => {
				dispose = channel.onMessage(value => {
					if (value === 'online') {
						resolve(channel);
					}
				});
